...
sudo wget -O "/usr/local/bin/docker-compose" "https://github.com/docker/compose/releases/download/1.9.0/docker-compose-Linux-x86_64"
...
sudo rm -rf /var/git/${REPO_NAME}.git /var/git/${REPO_NAME}
sudo mkdir -p /var/git/${REPO_NAME}.git /var/git/${REPO_NAME}
sudo git --git-dir=/var/git/${REPO_NAME}.git --bare init

sudo mv /tmp/${REPO_NAME} /var/git/${REPO_NAME}.git/hooks/post-receive
sudo chmod +x /var/git/${REPO_NAME}.git/hooks/post-receive
sudo chown ${SSH_USER}:${SSH_USER} -R /var/git/${REPO_NAME}.git /var/git/${REPO_NAME}
  '"
  echo "done!"
}
...
function help_menu () {
cat << EOF
Usage: ${0} (-h | -S | -u | -k | -s | -d [docker_ver] | -dc | -l | -g | -f | -a [docker_ver])
